...
- [x] 4.1 实现注册页：邮箱+密码注册，密码强度校验（<6 拒绝），邮箱重复提示
- [ ] 4.2 实现邮箱验证流程：注册后发验证邮件，未确认邮箱不能登录/发布，登录时给出验证提示
- [x] 4.3 实现登录页：邮箱+密码登录成功后跳转动态流
- [x] 4.4 实现 Google OAuth 登录入口与 `/auth/callback` 回调换取 session（可选能力）
- [x] 4.5 实现登出：清除会话回到游客状态
- [x] 4.6 实现 `lib/domain/auth`：服务端读取当前用户/会话的统一工具
- [x] 4.7 实现未登录访问受保护页面（/posts/new、/settings/profile）时重定向登录页（带回跳参数）
- [ ] 4.8 验证邮箱账号与同邮箱 Google 账号为两个独立身份（MVP 不合并）

## 5. 用户资料能力（user-profile）

- [x] 5.1 实现 `lib/domain/profile`：读取与更新资料
- [x] 5.2 实现 `lib/validation` 中资料校验（昵称 1..30、简介 ≤200）的 zod schema
- [x] 5.3 为资料校验 schema 写单元测试（合法/空昵称/超长昵称/超长简介）
- [x] 5.4 实现资料编辑页 `/settings/profile`：保存昵称与简介，非法输入拒绝并提示
- [x] 5.5 实现头像占位逻辑（未设置头像时用占位图）
...
- [x] 7.3 实现 `lib/domain/image`：图片按上传顺序上传到 Storage（路径 images/{uid}/{postId}/{n}.{ext}）与校验
- [x] 7.4 实现 `lib/domain/post` 的 createPost：鉴权→校验→调 moderateContent→传图→事务写 posts+post_images
- [x] 7.5 实现写库失败时尽力清理已上传图片（best-effort），避免孤儿文件
- [x] 7.6 实现发布页 `/posts/new`：表单（正文+多图）、Server Action 提交、成功跳转详情（未登录访问的重定向见 4.7）

## 8. 动态流与详情能力（post-feed）

- [x] 8.1 实现 `lib/domain/post` 的 listPosts：未删除、时间倒序、分页（默认 20 条）
- [x] 8.2 实现动态流首页 `/`：卡片展示作者昵称/时间/正文摘要（前 100 字截断）/首图，加载更多
- [x] 8.3 实现空状态：无动态时引导发布第一条
- [x] 8.4 实现 `lib/domain/post` 的 getPost：取单条含全部图片
- [x] 8.5 实现动态详情页 `/posts/[id]`：完整正文+全部图片+作者+时间
- [x] 8.6 实现详情页对不存在/已删除动态展示"内容不存在"

## 9. 个人主页与管理能力（post-management）

- [x] 9.1 实现 `lib/domain/post` 的 listMyPosts：当前用户未删除动态、时间倒序
- [x] 9.2 实现个人主页 `/u/[id]`：展示该用户动态列表
- [x] 9.3 实现软删除 Server Action：写 deleted_at，仅作者可操作
...
- [x] 10.4 运行 lint / build / 单元测试全部通过，清理临时文件
- [x] 10.5 补充服务条款占位静态页，完善 README 部署说明

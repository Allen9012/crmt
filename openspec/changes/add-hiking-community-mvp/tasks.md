...
- [ ] 1.1 用 pnpm 初始化 Next.js（TypeScript + App Router + Tailwind + src 目录），可本地启动空白页
- [ ] 1.2 引入并初始化 shadcn/ui，配置基础主题与组件目录
- [ ] 1.3 配置 ESLint + Prettier + tsconfig 严格模式，确保 lint/build 通过
- [ ] 1.4 添加 vitest 测试框架与示例用例，确保 `pnpm test` 可运行
- [ ] 1.5 创建 `.env.example`，列出 NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY / SUPABASE_SERVICE_ROLE_KEY
- [ ] 1.6 编写 README：本地启动、Supabase 配置、数据分层约定、环境变量说明

## 2. Supabase 接入与数据模型

- [ ] 2.0 安装并配置 Supabase CLI，创建/link 云端项目，初始化本地 `supabase/` 目录；启用邮箱 provider 并保留邮箱验证
- [ ] 2.1 实现 `lib/supabase/server.ts` 与 `lib/supabase/browser.ts`（基于 @supabase/ssr，cookie 会话）
- [ ] 2.2 编写 migration：创建 `profiles` 表（id PK=auth.users.id, nickname, bio, avatar_path, timestamps）
- [ ] 2.3 编写 migration：创建 `posts` 表（id, author_id FK, body, meta jsonb, timestamps, deleted_at）
- [ ] 2.4 编写 migration：创建 `post_images` 表（id, post_id FK cascade, storage_path, sort_order, created_at）
- [ ] 2.5 编写 migration：创建索引（posts 倒序 where deleted_at is null；posts 按 author；post_images 按 post_id,sort_order）
- [ ] 2.6 编写触发器：监听 auth.users 插入自动建 profile（nickname=邮箱前缀），覆盖邮箱与 Google 首登
- [ ] 2.7 创建 Storage `images` bucket，配置读策略（public 读）与写策略（仅登录用户、路径限 `{uid}/` 前缀）
- [ ] 2.8 用 `supabase gen types` 生成 DB 类型到 `src/types`，供领域层引用

## 3. RLS 行级权限（安全核心，需多账号验证）

- [ ] 3.1 编写 profiles RLS：select 公开；insert/update 仅 auth.uid()=id
- [ ] 3.2 编写 posts RLS：select 仅 deleted_at is null（作者可读自己全部）；insert 仅 author_id=auth.uid()；update 仅作者；禁止物理 delete
- [ ] 3.3 编写 post_images RLS：select 跟随 post 可见性；insert/delete 仅关联 post 的作者
...
- [ ] 5.2 实现 `lib/validation` 中资料校验（昵称 1..30、简介 ≤200）的 zod schema
- [ ] 5.3 为资料校验 schema 写单元测试（合法/空昵称/超长昵称/超长简介）
- [ ] 5.4 实现资料编辑页 `/settings/profile`：保存昵称与简介，非法输入拒绝并提示
- [ ] 5.5 实现头像占位逻辑（未设置头像时用占位图）

## 6. 内容审核责任链骨架（content-moderation）

- [ ] 6.1 定义 `lib/domain/moderation`：ModerationContext / ModerationResult / ModerationHandler 接口
- [ ] 6.2 实现 `moderateContent` 责任链执行器，空链默认返回 allowed:true
- [ ] 6.3 为责任链执行器写单元测试（空链放行、模拟拒绝节点阻断），验证新增节点不改调用方

## 7. 发布动态能力（post-publishing）

- [ ] 7.1 实现 `lib/validation` 中发帖校验（正文 ≤2000、空内容拒绝、图片数 ≤9、单图 ≤10MB 且为图片类型）
- [ ] 7.2 为发帖校验写单元测试（空内容拒绝、正文超长、图片超 9 张、单图超限/非图片）
